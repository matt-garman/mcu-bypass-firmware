...
## Proof obligations for a firmware change

Any edit under `src/` — including one whose only intent is tidiness — has to
re-establish every property the layers below establish, and "the tests passed"
does not say which of them were re-established. This section is the inverse of
the tables that follow: the property first, then the commands whose evidence
discharges it.

Read a green run with two facts in hand. A lane that needs XC8, a PIC device
pack, gpsim/libgpsim, the ATtiny_DFP or the patched `yasimavr` venv skips by
name when its input is absent and the run still ends `0 failures`;
`STRICT_TOOLS=1` turns each of those skips into a failure, and
`make test-strict-tools` is what proves it really does. And three `make test`
members named for an oracle check the *checker* rather than the image:
`make test-attiny202-output-oracle`, `make test-attiny202-fault-oracle` and
`make test-attiny202-delay-oracle` drive synthetic input so the AVR-XT checkers
stay guarded on a host that cannot build the image. The image itself is measured
by `make attiny202-sim`, `make attiny202-fault` and
`make attiny202-delay-oracle`.

| obligation | what a firmware change must re-establish | commands whose evidence discharges it |
|---|---|---|
| Complete MCU/output-stage matrix | Every supported part and output stage still builds and still runs, not only the default pair. | `make test-target-matrix` and `make test-target-lane-markers` prove a complete matrix runs exactly once per variant and that a skipped, crashed or failing-but-zero-exit lane withholds aggregate success; `make test-variant-map-contract` proves no variant is half-declared; `make test-pic-build` and `make test-attiny202-build` prove a partial compiler output cannot become an image. The matrix is run on target by `make attiny202-test-target`, `make pic10f322-test-target-variants`, `make pic12f675-test-target-variants` and `make pic10f320-test-target-variants`; `make pic10f320-test-host-variants` is the one all-variant sweep that needs no target toolchain. |
| Exact selector/driver correspondence | Each variant still compiles exactly the driver its selector names, and a bad, absent or unsupported selector is refused rather than quietly building less. | `make test-variant-selector-guard`, `make test-analyze-variant-guard`, and `make test-attiny202-build`, which pins each supported variant to its exact selector/driver pair. Under the real device headers the correspondence is re-checked by `make attiny202-analyze`, `make pic10f322-analyze`, `make pic12f675-analyze` and `make pic10f320-analyze`, each of which compiles the shell, the pure core and every matching-selector driver. |
| Byte identity where no code-generation change is intended | An edit meant to be comment-, layout- or preprocessor-only emits the same image bytes; any change in bytes is intended, reviewed and requalified. | No goal establishes this, because it compares two trees rather than reading one: build and keep the complete image set before the edit, rebuild after, and compare. What makes that comparison mean anything is `make test-avr-build-rebuild`, `make test-pic-build-rebuild` and `make test-workload-rebuild`, which prove a changed input really does invalidate the artifacts that depend on it — an unchanged HEX is a result only where a changed one would have been rebuilt. `make test-pic10f320-expected-images` pins that part's set by name. |
| Resource envelopes | Flash, static data, AVR stack frames, runtime stack high-water where available, and the PIC 8-level hardware return stack all still fit their reviewed budgets. | `make test-stack-bound` bounds fresh Classic AVR frames and `make attiny202-test-stack-bound` the AVR-XT shell's; the AVR runtime high-water comes from the canary gate inside `make test-sim-attiny13a` and `make test-sim-tinyx5`; `make test-flash-budget` bounds Classic AVR image size. The PIC return stack is bounded by `make pic10f322-test-stack-bound`, `make pic10f320-test-stack-bound` and `make pic12f675-test-stack-bound`, with `make test-pic10f320-return-stack-oracle` reading the final HEX as a second, independent witness. `make test-resource-tables` then requires every image the tree has built to sit under the ceiling its own build gate enforces, and `make test-stack-bound-regression`, `make test-stack-bound-pic-regression` and `make test-flash-budget-regression` prove those checkers reject missing, malformed and partial evidence instead of passing it unmeasured. |
| Tick formulas, delay-body cycles, delivered pulse widths, ISR duty, watchdog pet-to-pet bounds | The compile-time budget still holds with every term load-bearing, and the widths the image really delivers still match design. | `make test-static-assert-guards` holds the budget in every MCU shell; `make test-fault-wdt-note-contract` keeps each part's watchdog facts inside the macro that publishes them; `make test-soak-timing` holds the soak's own sizing contract. Delivered widths are measured on the real image: `make test-sim-attiny13a` and `make test-sim-tinyx5` check the longest `wdr`-to-`wdr` interval against the compiled-in budget, `make attiny202-delay-oracle` recovers the relay and mute widths from the disassembled `_delay_ms` loop and bounds the sole ISR's instruction count, and `make pic10f322-test-io`, `make pic12f675-test-io` and `make pic10f320-test-io` measure transitions and pulse widths in simulator cycles. |
| Relay emergency de-energization and recovery electrical sequence | On a sanity-gate fault the firmware commands both coil outputs idle in one target operation. Where reset is modeled, recovery keeps SET inactive, commands a RESET pulse of at least the datasheet minimum, settles logical state and LED to BYPASS/dark, and leaves both coils idle. Separate timing lanes establish the nominal design width; no lane observes armature or contact position. | Without a target toolchain the PIC10F320 host lanes prove the pre-spin clear and ordinary actuation separately, not a fault-linked reset pulse. On target, `make pic10f322-test-fault`, `make pic12f675-test-fault` and `make pic10f320-test-fault-target` require the single-write clear sampled every instruction plus the minimum recovery RESET pulse; target-I/O separately checks design width. `make attiny202-fault` requires modeled PA2/PA3 low with OUT/DIR/PINnCTRL canonical before the spin. |
| Fault injection across target-specific guarded state | Every guarded SFR, direction, latch, configuration, pull-up and `ctx_` location still forces the designed watchdog recovery — and the documented unguarded ones still do not. | `make test-fault-inject` on the tinyx5 family, where simavr models the WDT system reset, and `make pic10f320-test-host-variants` on the host. On target, `make pic10f322-test-fault`, `make pic12f675-test-fault`, `make pic10f320-test-fault-target` and `make attiny202-fault`, each with fail-closed injection accounting and an expected per-variant check count. |
| Lock-step and equivalence against `src/bypass_pure.c` | The shipping core is still the only algorithm: each shell's live state matches it tick for tick, and the PIC10F320's hand-inlined copy still agrees with it. | `make test-host` against the independently written host model, and `make test-cbmc`, `make test-model-check` and `make test-symbolic` over the invariant-valid state space; `make pic10f320-test-host-variants` runs the host equivalence harness for the inlined part. On target, `make pic10f322-test-lockstep`, `make pic12f675-test-lockstep`, `make pic10f320-test-lockstep` and `make attiny202-lockstep` compare live SRAM `ctx_` against that same core. `make test-lockstep-progress` proves a stalled lock-step aborts instead of reporting a short run as a pass, and `make test-attiny202-model-ffi` proves the ctypes bridge reaches the shipping core. |
| Negative compile guards and diagnostics | Required per-file guard declaration counts remain fixed, and selected high-consequence predicates fail on their broken input with their own message. This is not semantic mutation coverage of every assertion. | `make test-static-assert-guards` exercises the classic-AVR/shared roster and separately censuses every guard in every MCU shell; `make test-attiny202-guard-mutations` and `make test-pic-guard-mutations` exercise target-local pin, clock, layout, threshold and timing-budget representatives under the real toolchains. The census catches changed declaration counts but not an unmutated predicate replaced by unconditional truth. Both target lanes are registered in `test/test_strict_tools.sh`, so `STRICT_TOOLS=1` fails rather than removing their coverage. `test/test_target_guard_mutations.sh`, which backs both, retains each formerly accepted invalid configuration as an exact diagnostic fixture, so closing a gap does not erase its negative test. |
| MISRA and static analysis under all materially distinct configurations | Every configuration that compiles differently is analysed, not only the default. | `make analyze`, and `make attiny202-analyze`, `make pic10f322-analyze`, `make pic12f675-analyze` and `make pic10f320-analyze` under the real device headers. `make test-analysis-matrix` holds the matrix itself complete, and `make test-misra-output-contract` makes the diagnostic parser fail closed, so an analyser that has stopped reporting cannot pass as clean. |
| Preservation of independent pin/output/release identity oracles | The pin, output and release-identity second opinions still exist and can still disagree. | `make test-deliberate-duplication` is the mechanical floor: it fails by name on any fold of a registered duplication, and its message carries the opinion the fold would destroy. `make test-pinout-alignment`, `make test-attiny202-output-oracle` and `make test-release-images` hold the individual oracles. |

Whether these lanes would notice a change at all is the mutation lane's
question, and it runs in `make test-long` rather than in `make test`.

Clearing the list is necessary and not sufficient.
`make test-deliberate-duplication` covers structure; the rest of the list asks
...
| Fault response | `pic10f322-test-fault`, `pic12f675-test-fault` | Every guarded direction, output-latch, configuration, pull-up and `ctx_` corruption produces the designed watchdog recovery, injected at the behaviourally identified main-loop `CLRWDT`. Relay variants additionally require both coils commanded idle *in one output write* — sampled every instruction, so a two-step clear fails — before the reset spin, and a measured RESET-coil pulse of at least the datasheet minimum after it, from both a settled BYPASS and a settled ENGAGED start. Target-I/O separately checks nominal design width. PIC12F675 also injects through physical pin nodes and its comparator-mode neighborhood, so low GPIO intent cannot masquerade as an idle modeled pad, and requires parked GP4 low at the spin. No lane observes armature or contact position. Register identity, injection readback, and the expected per-variant check count are fail-closed test invariants. | libgpsim |
...
| PIC harness rebuilds | `test-pic-build-rebuild` | Soak binaries compile their workload sizing in as `-D` flags, so changed and identical duration requests must both recompile with the selected image and independently maintained block value. Direct fault and lock-step binaries must also rebuild their selected image and assembly/symbol sidecars when those generated inputs are absent. | Bash + fake XC8/injector/c++ |
...
profile's exact final-check contract remains in `test/test_pic_build.sh`, so an
incomplete profile or a missing rebuild arm fails instead of reporting a smaller
subset as green.
...
| layer | target | what it proves | substrate |
|---|---|---|---|
| Release image reproduction | `test-release-images` | Committed images, `SHA256SUMS`, and fresh builds must each match the Makefile's canonical image set before byte reproduction is accepted. `SHA256SUMS` must partition into exactly three declared sets -- images, required helpers and provenance files -- so an entry belonging to none of them is a file that got signed without being reviewed, and a declared file that is absent is provenance that quietly stopped being covered. Which contract applies is read from the release's own `QUALIFICATION` format field, because this verifier also runs against published directories on every PIC12F675 field programming: all three published eras (no `QUALIFICATION`, `format=1`, `format=3`) signed images and helpers only and are held to that, and only `format>=4` requires provenance inside the signature. Production pins the repository Makefile and ignores inherited overrides, Make flags, command-line assignments, environment precedence, and injected makefiles. The canonical set is itself held to an `override` identity pin computed from disjoint inputs: a set wider or narrower than the pin, or an empty pin, is rejected, while the same members in another order are not drift. | Bash + synthetic Makefiles |
| Release provenance contract | `test-release-provenance` | Final images are rechecked immediately before staging, including a HEX digest regenerated from the validated ELFs and a post-copy comparison that dominates `SHA256SUMS`. Tag CI snapshots clean-build image paths, requires four-way reproduction against the committed files, checksums and canonical set, then freezes the image/helper/metadata inventory before later gates run. The publication shell rechecks the remote tag, inventory digest, detached signature and strict checksums immediately before publishing, so synthetic additions, mutations, missing helpers and failed checks cannot reach it. The flashing headings the renderer writes and the verifier extracts by are pinned on both sides, as are the two `bash -n` passes over the published commands. | Bash + fake git/gh |
| Release qualification contract | `test-release-qualification` | Publication requires clean production metadata, the exact canonical evidence set, source-bound resource/toolchain records, one complete PIC12F675 matrix, and one complete result for every canonical release soak combination. A release requires the committed soak record to cover the images it built, at its own duration floor, and publishes the commit whose run produced it. Signed provenance must match its evidence in both directions. `evidence/INDEX` is checked against the Makefile's independent role map and bound through `evidence_index_sha256`; each build/target operation seals its exact transcript payload in `EVIDENCE_RESULT format=2`, so equal-size substitutions and self-consistent false digests fail independent rehashing. Resource rows and programming commands are re-derived rather than trusted; resource records also require closed schemas and exact part/variant coverage. PIC12F675 remains restricted to its guarded transaction, and its release disposition is read from a `pic12f675-disposition` marker block rather than from the first line of a paragraph. The safety-relevant multi-MCU distinctions are required as ordered patterns that keep every figure and part association while letting the prose between them move; each of those rules is proved to be anchored on the fact it names by deleting its own match and requiring it to stop matching. That rule table is read once by both passes rather than written out a second time for the negative one, so a rule cannot be added to the positive half alone and end up with no negative coverage while the suite stays green. The PIC12F675's open silicon-only risks are held two ways: their numbers are an interface the Makefile, the CI notes and the release documentation cite, so the enumeration is pinned complete and in order, while each item is held only to the terms of its **defining sentence** -- the whole item will not do, because a body names its own subject many times over and stays satisfied after the headline naming it is gone. Both halves carry their own spoiled cases, and an accept case restates all four risks in another voice. Every part whose per-image `pk2cmd` command a release publishes must have a declared Makefile authority to check it against, each pinned to its own part's variables -- stated as a completeness rule over what the producer emits rather than as a list of today's two parts, because the PIC10F320 published a command with nothing to compare it with for four releases. Each source-checkout command is bound to its image's exact MCU-specific Make goal, sole variant-selector name, and complete variant value; prefixed, foreign, duplicate and conflicting assignments are rejected independently by the producer, renderer and retained-manifest verifier. | Bash + Python + synthetic retained evidence |
| Release preflight contract | `test-release-preflight` | The real release step 0 reaches its final executable-version probe without cleaning, building, staging, or changing tracked worktree content. A versioned run first requires one nonempty dated changelog section, exact comparison links, and one bounded current-release declaration -- in `release/README.md` and nowhere else -- that agrees with the canonical image/soak inventory and part/shell topology; malformed bounds and stale exact fields fail before release scratch is created, while historical prose outside the bounds stays valid. A second declaration in any other current document is refused by name, including one that happens to agree; shipped release directories and declared branch-only working documents are not copies to maintain and are exempt, the declaration being read in either documentation markup and either emphasis spelling so that a root-level working document cannot reach a release by being AsciiDoc. A bounded declaration may not claim retained evidence the tree does not contain, and only a nonempty regular non-symlink `QUALIFICATION` suppresses its pre-tag transition disclosure. The gate's own baseline is hermetic: inherited build-input names are cleared once before the first case, so a caller that exports one cannot turn the clean-configuration control into a refusal. It also runs the live-tree documentation contracts, each against spoiled copies of the shipped documents: the field-use/qualification split, the fenced claims a consolidation drops most easily, and the current-fact boundary. A fenced claim is bound by a named marker pair and held to the terms it must still state -- not to a sentence -- so the prose inside a fence stays the author's, while deleting the fence, emptying it, leaving it unclosed, inverting the claim, or keeping the denial and dropping what it denies all fail with the marker named. The published host compiler floor is held the same way: each publishing document must state the enforced number beside a host `gcc` mention rather than use one of two accepted sentences, so a reworded floor passes, a cross-compiler version does not, and a bumped floor nobody republished fails. Every such claim also carries an accept case, the same commitment rewritten in another voice, which is what fails if a gate ever quietly re-pins prose. Numeric release topology is confined to the bounded declaration; live simavr timing, PIC loop-cycle/current and XC8 optimization results cannot return as unbound design/toolchain facts, while the historical PIC10F320 fit decision is fenced as the overrun it records rather than pinned as a sentence, and `DESIGN_DOCUMENTATION.adoc` carries no date and no source revision at all -- the binding that a misplaced measurement asks for cannot return in place of moving the measurement out. Those current-fact rules are a separate validator from the fenced claims and are checked here alone: a violation must be rejected on the tree and must NOT reach the contract the release path calls, so a drifted design sentence cannot stop a release, and the release script is read to prove it never calls them. Release topology -- the part, image, soak-combination, modular-target and shell-source counts -- is held by a third validator on the same terms, and its numbers are derived rather than written: they come from the canonical release sets and the shipping shells, the bounded declaration is required to state every one of them, and no other durable document may state any of them beside the thing it counts. The owner and the changelog are the two documents it does not read, a fenced region carries the one registered exemption, and a fence that has been deleted fails like any other. The cases prove the patterns follow the derived value rather than a literal by holding the same sentence to a count this tree does not have. The PIC12F675 source-checkout transaction likewise remains in `release/README.md`; `TOOLCHAIN.adoc` must link to it without republishing transaction goals or evidence-path policy. While the validation log says no controlled record exists, no durable document may attribute that qualification to the firmware, an image or a part; naming the wording in a code span is not making the claim, and the ban lifts with the sentinel. The printed handoff must sign, register, verify, stage, commit and then PROVE in that order, and the signed-tag command may appear in `scripts/verify-release-artifact-commit.sh` alone -- a tag recipe printed by a run that has not proved its artifact commit is the refusal turned back into advice. | Bash + fake toolchain |
| Release history and signature | `test-release-history` | The tag event must peel to a single-parent child of the exact qualified source. For parents carrying the publication registry, that child may change only its release directory and one exact canonical registry append; legacy parents retain their release-subtree-only contract. A synthetic future RC followed by its final release must satisfy this history rule and the complete immutability gate together; semantic release ordering moves the newest-release exemption to the final and makes the RC's continuity declaration mandatory. Appending that release also moves the exemption off the newest release this repository published, so the fixture declares its image continuity in the appended release's source commit, the way a real superseding release does; the counts come from the gate's own ordering and signed lists, and a wrong one fails. `SHA256SUMS.asc` and the remote annotated tag must verify against the pinned full-fingerprint key in an isolated keyring; altered bytes, missing or wrong-key signatures, lightweight, unsigned, replaced and moved tags are rejected immediately before publication. The frozen publication oracle binds an exact expected asset set to descriptor-read identities, sizes and digests, so every mutation, addition, removal, rename, unsafe name, symlink and ordering drift fails closed. | scratch Git + fake gh |
| Release artifact commit | `test-release-artifact-commit` | The last gate before a tag exists. `scripts/verify-release-artifact-commit.sh` must refuse an artifact commit whose tracked files differ from `HEAD`, whose version is already tagged, whose release directory is missing, unsigned, or dry-run staged, whose checksum signature or history rule fails, or whose gate inventory is empty or unreadable -- and every refusal must print no tag and no push command, because this script is the only thing that prints them. The publishable case must dispatch through the `release-artifact-gates` goal, reach every gate in `RELEASE_ARTIFACT_GATES` under `STRICT_TOOLS=1` and with no pins, and print both commands. Its `--allow-dry-run` rehearsal mode must relax exactly two things -- the dry-run banner and the signature a dry run has not made -- while still refusing every other case, requiring the banner it permits, and printing no tag or push command. | Bash + scratch Git + stub verifiers |
| Release artifact rehearsal | `test-release-rehearsal` | `scripts/rehearse-artifact-commit.sh` must assemble, in a scratch clone, exactly the commit a tag would name: a single-parent child of the commit `QUALIFICATION` records -- not the branch tip, which may have moved -- carrying the staged directory verbatim plus one appended publication registration, and nothing else, with the clone left clean for the verifier it then runs in rehearsal mode. It must refuse a staging that is not a dry run, one with no source commit or a source commit this repository does not have, a version already tagged, a source tree that already contains the release directory, and it must fail when the verifier does, keeping the clone. Every case re-asserts that the real repository's HEAD and working tree are untouched. | Bash + scratch Git + stub verifier |
| Published release immutability | `test-published-release-immutability` | The signed tag is the immutable identity of the original release; the retained development-branch copy must match it except at explicitly registered amendment paths. Every release's own `SHA256SUMS` is re-verified, while files outside that list are held to `published_release_digests.txt`. A future artifact commit must append exactly the canonical registry block generated after checksum signing, and the two lists must partition each directory exactly. Nothing under `release/` may be a symbolic link. Where tags are present, direct tree comparison rejects every difference outside the amendment register and also rejects a registered difference that is no longer present. The v0.9.0-v0.9.2 TMUX safety warnings are confined to six unsigned result-record files; their images, checksums, signatures, tags, and original publication identity remain unchanged. Image continuity is independently declared and recomputed so deliberate republication cannot hide accidental restaging; prerelease ordering determines which newest release may temporarily owe that declaration. | Python + Git tags where present |
...
| `test-reference-contract` | Every section citation, relative link and anchor in the live tree still resolves. A section number is reserved for an external document, which is renumbered by its publisher rather than by us; a repository document is cited by name and anchor, so the link half can check it. `GOVERNANCE.md`'s lifecycle table, found by its `document-lifecycle` marker pair rather than by a heading whose wording belongs to the document, must classify every mapped durable authority exactly once and must classify per-release file roles rather than overlapping whole directories. Written after the PIC10F320 merge plan was deleted with 43 section citations still pointing at it from the Makefile, the release scripts, the CI workflows and the test suite. |
| `test-release-prepare` | The release preparer writes the derived release lines -- the dated heading, the re-opened `[Unreleased]` section, both compare links, the bounded contract line and the pre-tag transition line -- and writes no prose. Its centrepiece replays the `v0.9.12` cut from this repository's own history and requires byte identity with the four commits that produced those seven lines by hand; the rest hold the boundary, including that every refusal leaves both documents untouched rather than half-prepared. |
| `test-resource-tables` | Every image the tree has already built measures at or below the reviewed ceiling its own build gate enforces, and no reviewed ceiling is wider than the silicon it bounds. The ceilings are read from the Makefile that declares them, with exactly one assignment required before its decimal-constant form is accepted, so no document has to restate a measurement and no later computed reassignment can change the effective policy unseen. The release-only strict mode additionally requires the complete released image set plus retained RAM and stack evidence, with every record checked against its own arithmetic and against the limit it reports, bound to the source HEAD. Strict mode emits every figure it measured as one `RESOURCE_*` record per published value rather than reducing them to counts, and the release renders its resource tables from those records instead of deriving its own. Three agreements are required before a figure is published: the two AVR static-data oracles (allocated ELF sections, and the SRAM map the simulated canary gate derives from the device's symbols) must agree, a part's variants must agree before one static figure stands for the part, and the PIC12F675 qualified build and its reproducibility rebuild must agree on Data space. |
...
| `test-static-assert-guards` | Every modular shell carries an anchored, uncommented direct include of the shared compile-time checks. Selected reachable shared and Classic-AVR predicates fail when their inputs are broken. Near-bound fixtures pin the watchdog pet-to-pet budget to its exact millisecond, convert wall-time ISR duty independently, prove the ISR, tick and loop terms are all load-bearing, and reject the former mixed formula. Its separate census counts every guard in every MCU shell, including the ones only a target toolchain can compile, so a changed per-file declaration count is reported on a host that has no XC8; the census does not prove the semantics of unmutated predicates. |
| `test-attiny202-guard-mutations` | Selected ATtiny202 pin ordinals, all three enum widths, tick derivation, clock and watchdog budgets fail the build when broken -- under avr-gcc with the vendored device pack, because the pin asserts read `<avr/io.h>` and the budget is computed from that part's floor, tick and ISR duty. Losing the part selector is caught rather than silently taken for a classic AVR. |
| `test-pic-guard-mutations` | The corresponding selected proof for the PIC10F322, PIC12F675 and self-contained PIC10F320 shells under XC8: representative pin ordinals against the device pack, the GP3/GP4 spare-pin family, `_XTAL_FREQ`, a PIC10F320 duplicated threshold invariant and its branch-local pin guards, and each part's watchdog budget pinned to its exact millisecond. Configurations the firmware still accepts in silence are recorded here too, so the guard that closes one has to be proven load-bearing when it lands. |
| `test-deliberate-duplication` | The register of duplications that are second opinions rather than untidiness, and the structural witness for each: the self-contained PIC10F320 shell reaches no header this tree owns, every part states its own pin ordinals and its own four watchdog terms as literals, each shell owns its main loop, the AVR shells stay interrupt-driven and the PIC shells polled, the clock is only ever tested by the firmware and never supplied by it, the PIC harnesses keep their register facts literal, both PIC return-stack witnesses read different artifacts, every verification layer is still wired to a subject of its own, and the pinned release identity is still spelled in literals rather than composed from the build variables it exists to check. Lexical, so it reports a fold on any host. |
...
| `test-strict-tools` | The skip-versus-strict policy holds for the host lanes and all three PIC parts, so `STRICT_TOOLS=1` really converts every clean skip into a failure. It also holds `test-cbmc` to the proofs `test/formal/test_cbmc.c` defines: each defined proof is dispatched exactly once and nothing undefined is, compared by name, so a proof left out of every Makefile list fails naming it. |
