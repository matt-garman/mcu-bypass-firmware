...
| Negative compile guards and diagnostics | Every static assertion still fails the build on its own broken input, with its own message. | `make test-static-assert-guards` for the classic-AVR lane and for the census of every guard in every MCU shell; `make test-attiny202-guard-mutations` and `make test-pic-guard-mutations` break one guard input at a time under the real toolchains and require that guard's own diagnostic. Both are registered in `test/test_strict_tools.sh`, so `STRICT_TOOLS=1` fails rather than removing the coverage. `test/test_target_guard_mutations.sh`, which backs both, also records the configurations the firmware still accepts in silence, and each of those rows fails when the guard closing it lands — so a new guard has to be proven load-bearing in the change that adds it. |
