...
| Published release immutability | `test-published-release-immutability` | A published release is what its recipients already hold, so the copy under `release/` is the same object rather than a draft of it. Every release's own `SHA256SUMS` is re-verified, so offline integrity is checked rather than assumed from having held once. The files no release signs -- the evidence logs, `QUALIFICATION`, the manifests and the detached signatures themselves, none of them rebuildable from source -- are held to `published_release_digests.txt`, an ordinary `sha256sum -c` file so the central claim reproduces with coreutils alone. A future artifact commit must append exactly the canonical block generated after checksum signing; the history gate preserves the registry prefix and independently regenerates the suffix. The two lists must partition each directory exactly, so a file added to a published release is covered by one or the other and never by neither, and no image can drop out of the signed list into the merely-recorded one. Nothing under `release/` may be a symbolic link: every check above reads content through the path, so a published image swapped for a link to identical bytes elsewhere would satisfy all of them, and publication already refuses symlinked assets, inventories and signatures. Where the clone has tags, each tag tree is compared directly: an edit that updates both of this tree's lists to agree with itself is still caught. The one amendment ever made to a published release, the v0.9.0-v0.9.2 TMUX polarity errata, is registered with its reason, the markers that must survive in it, the anchor it sends the reader to, and the check that it touched no file any verifier reads. What each release did to the images the one before it published -- how many it republished byte for byte, how many it rebuilt, and why -- is declared and recomputed from the two signed lists, so a build that restages bytes it did not produce cannot pass as the ordinary republication this project's releases deliberately are. The newest release is not required to have declared yet -- its counts cannot exist in the commit that publishes it -- but a declaration it does carry is checked, the requirement lands the moment the next release supersedes it, and until then the passing summary names the release that owes one. | Python + Git tags where present |
...
| `test-reference-contract` | Every section citation, relative link and anchor in the live tree still resolves. A section number is reserved for an external document, which is renumbered by its publisher rather than by us; a repository document is cited by name and anchor, so the link half can check it. Written after the PIC10F320 merge plan was deleted with 43 section citations still pointing at it from the Makefile, the release scripts, the CI workflows and the test suite. |
