...
| Relay emergency de-energization and complete recovery actuation | On a sanity-gate fault both coils go de-energized in one output write, and the RESET coil gets a full-width actuation after the reset, from both a settled BYPASS and a settled ENGAGED start. | Without a target toolchain only the PIC10F320 lane proves it: `make pic10f320-test-host-variants` runs the actuation sequencer and the host fault injector over every variant, and its relay arm requires both coils cleared in one `LATA` write. On target, `make pic10f322-test-fault`, `make pic12f675-test-fault` and `make pic10f320-test-fault-target` require the single-write clear sampled every instruction plus a measured full-width RESET-coil actuation, and `make attiny202-fault` requires modeled PA2/PA3 low with OUT/DIR/PINnCTRL canonical before the spin. |
