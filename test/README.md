...
| Release qualification contract | `test-release-qualification` | Publication requires clean production metadata, the exact canonical 37-file evidence set, source-bound hashed resource evidence, one complete PIC12F675 image matrix bound to the released HEX and checksums, and one identity-, duration- and counter-bearing result for each of 18 release soak combinations. Both publishable modes are told apart: `production` keeps its 24-hour floor and `express` its 1-hour floor, and each is rejected if it carries the other's manifest banner or none at all, so the recorded mode and the human-readable document can never disagree. The rendered validation prose is held to its substrate claim in both directions: the signed `MANIFEST.md` is published verbatim as the release body, so its simulator lanes must be called modeled-pin checks and may not claim physical output evidence. The per-release `README.md`, which no verifier read before `QUALIFICATION` format=4, is held to the same authority as the manifest for its version heading and its qualification banner. `QUALIFICATION`, `MANIFEST.md` and `README.md` must each be listed in `SHA256SUMS` and must hash to the value listed, so a provenance file edited after the release was sealed is caught even when it contradicts nothing else. The `MANIFEST.md` toolchain table is rendered from `evidence/toolchain.txt` rather than authored, and held back to it in both directions: every recorded tool must appear as a row, and the table may carry no row the record does not justify, so neither a dropped tool nor a version edited in place nor a smuggled-in entry survives. Every retained file is listed in `evidence/INDEX` by role, size and terminal record, bound as `evidence_index_sha256` and checked in both directions against the Makefile's declared role map -- so a mislabelled member disagrees with a declaration it did not write, and an unlisted one is a row the verifier cannot satisfy. The thirteen build and target-test logs that previously had no content authority at all, 62% of the evidence tree by bytes, each carry an `EVIDENCE_RESULT format=1` record binding the log to the released commit, to its own name and to its own length, so a log kept from an earlier run, substituted for its neighbour, truncated or extended is rejected. Every resource figure the manifest publishes is re-derived from the `RESOURCE_*` records in `evidence/resource-tables.log` and must match exactly: the flash column is read out of each image row by a pinned column position, the static-RAM, stack, frame-bound, Data-space and return-stack rows are matched literally, each record must close on its own arithmetic, the record set must cover the canonical image set in both directions, and the terminal result's `records=` count must equal the figures actually present. A per-frame compiler bound may carry no high-water field, so the AVR-XT frame figure cannot be published as though it measured the deepest path. The published programming commands are re-derived from the staged manifest and run through `bash -n`, so a command that is not valid shell fails the release; each must name the image it is filed under, carry every fuse byte its own images row publishes, write that image to flash, verify what it wrote, and cover the canonical image set in both directions -- with PIC12F675 required to carry no per-image command at all. Source-checkout equivalents are held to being make invocations that select the variant their own image names. The regression also preserves the historical 28-file/15-soak boundary for v0.9.6-v0.9.8. | Bash + Python + synthetic retained evidence |
