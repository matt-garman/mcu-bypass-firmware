...
| gpsim process gate | `test-gpsim-wrappers` | Both functional wrappers require a positive decimal timeout, reject nonzero or killed gpsim runs even after complete snapshots, and fail rather than skip missing gpsim under `STRICT_TOOLS=1`. | Bash + fake gpsim |
...
| Lock-step progress regression | `test-lockstep-progress` | Both chip-specific adapters abort simulator stalls during settle, calibration, or completion immediately; the completion loop cannot spin forever on a frozen cycle counter. | host C++ + fake gpsim API |
