...
| Release artifact commit | `test-release-artifact-commit` | The last gate before a tag exists. `scripts/verify-release-artifact-commit.sh` must refuse an artifact commit whose tracked files differ from `HEAD`, whose version is already tagged, whose release directory is missing, unsigned, or dry-run staged, whose checksum signature or history rule fails, whose gate inventory is empty or unreadable, or whose release workflow declares none of the independent pins -- and every refusal must print no tag and no push command, because this script is the only thing that prints them. The publishable case must reach every gate in `RELEASE_ARTIFACT_GATES` under `STRICT_TOOLS=1` with those pins, and print both commands. | Bash + scratch Git + stub verifiers |
