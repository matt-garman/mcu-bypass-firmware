...
| `test-reference-contract` | Every section citation, relative link and anchor in the live tree still resolves. A section number is reserved for an external document, which is renumbered by its publisher rather than by us; a repository document is cited by name and anchor, so the link half can check it. README.md's lifecycle table must classify every mapped durable authority exactly once and must classify per-release file roles rather than overlapping whole directories. Written after the PIC10F320 merge plan was deleted with 43 section citations still pointing at it from the Makefile, the release scripts, the CI workflows and the test suite. |
