...
accommodations, including a cppcheck diagnostic that is not a MISRA rule. Every
file-based suppression is explicit in
...
project-wide report: it does not run the AVR-XT, PIC10F322, or PIC10F320
implementation files. Those lanes currently have no report-only companions.

### Additional MCU implementation files

| | AVR-XT (ATtiny202) | PIC10F322 | PIC10F320 |
|---|---|---|---|
...
- **SFR bitfield value-flow (`misra-config`).** cppcheck cannot fully
  ...
  PIC10F322 run suppresses it invocation-wide on the command line
...
- **Pinned configuration.** The PIC run forces `-D_10F322 -DBYPASS_MCU_PIC10F322`
  ...
  *unselected* AVR Classic and AVR-XT pin-map directives, so Rule 2.5 can fire on
  those inactive maps — covered as D-2 analyzer artifacts below.

## Compliance boundary

The compliance boundary is **this project's authored firmware source**:

- Eight C files: the four MCU implementation files
  (`bypass_mcu_avr_classic.c`, `bypass_mcu_avr_xt.c`,
  `bypass_mcu_pic10f322.c`, `bypass_mcu_pic10f320.c`), the shared
  `bypass_pure.c` core, and the three `bypass_output_*.c` drivers.
- Fourteen headers: `bypass_config.h`, `bypass_types.h`, `bypass_pure.h`,
  ...
  `bypass_static_assert.h`, `bypass_compile_checks.h`, the three
...
project-wide wildcard. A matching finding in a new file therefore fails the
gate. The other side of that granularity is important: another finding with the
...
from the target recipe, then remove `--suppressions-list=...`; for PIC10F322 also
remove its invocation-wide `--suppress=misra-config` to inspect that diagnostic.
