...
preprocessor configuration at a time. D-4 contains PIC10F320 analyzer
accommodations, including a cppcheck diagnostic that is not a MISRA rule. The
PIC10F322 and PIC12F675 shells require no deviation of their own — see
"Suppression review" below. Every file-based suppression is explicit in
...
gates on findings not covered by the documented suppression records below. It is
part of the `analyze` aggregate and therefore of `make test`. A caller can narrow
`VARIANTS`, so full driver coverage requires
...
companions — though for the two PIC shells that analyze clean against an empty
suppression file, the lane itself already *is* the unsuppressed report.
...
| Project suppressions consumed | three (D-1) | none | two (D-4) | none |
...
- **SFR bitfield value-flow (`misra-config`).** cppcheck cannot fully
  ...
  PIC10F322 and PIC12F675 runs suppress it invocation-wide on the command line
  (`--suppress=misra-config`). The PIC10F320 run instead scopes the diagnostic to
  its source file through the suppression list (D-4 below).
- **Pinned configuration.** The PIC run forces `-D_10F322 -DBYPASS_MCU_PIC10F322`
  ...
  branch of `bypass_output_common.h` is active. cppcheck still records
...
project-wide wildcard. A matching finding in a new **source** file therefore
fails the gate; see "Suppression review" below for what that qualifier costs. The other side of that granularity is important: another finding with the
...
from the target recipe, then remove `--suppressions-list=...`; for PIC10F322 and
PIC12F675 also remove their invocation-wide `--suppress=misra-config` to inspect
that diagnostic.
...
source-level noncompliance. D-4 records PIC10F320 analyzer accommodations, not an
actual MISRA deviation.

### Suppression review

Reviewed 2026-08-10, when the PIC12F675 shell joined the boundary. The method
was to re-run each lane against a *modified* suppression file and record what
changed, rather than to read the list and assume.

**Two shells need no project suppression at all.** `make pic10f322-analyze-misra`
and `make pic12f675-analyze-misra` both report clean against an **empty**
suppression file. Their only accommodation is the invocation-wide
`--suppress=misra-config` on the command line, and that covers an analyzer
modelling limit on adopted Microchip headers rather than anything authored here.
The reason is structural and worth stating: the PIC shells write SFRs as plain
identifiers (`GPIO = gpio_shadow_;`), where avr-libc's `_SFR_*` macros expand an
integer address to a volatile pointer — which is what produces the whole D-1
family. A PIC shell cannot inherit that deviation, so the PIC12F675 arrived
adding none.
...
**One finding, filed rather than fixed here.** With cppcheck 2.13.0,
...
D-2 and D-3 entry currently suppresses *output*, not a failure, and a new Rule
2.5 artifact in a new header would not fail a lane today.

That does not change the compliance posture — those entries record artifacts
that are false at project scope, and the argument for each is unchanged — but it
does mean the gate is weaker than "a matching finding in a new file fails the
gate" implies. The header entries are kept, and the PIC12F675 map was added
alongside the other three, so the policy is complete if a later cppcheck starts
gating header findings. Closing the gap itself is `TODO.md` T25-misra-header-gate.
...
| **Suppression scope** | `bypass_pins_avr_classic.h`, `bypass_pins_pic10f322.h`, `bypass_pins_avr_xt.h`, `bypass_config.h` |
...
another translation unit as unused. It can also retain directives from the two inactive pin-map branches selected through
`bypass_output_common.h`, producing findings against maps that are not active in
the current target configuration.

`bypass_config.h` has the same split. The shared core and three modular shells
...
### D-4 — PIC10F320 analyzer accommodations (`misra-config`, Rule 2.5)

**Finding IDs:** `misra-config` (a cppcheck diagnostic, not a MISRA rule) and
**Rule 2.5** (unused macro definition, Advisory).

**Suppression scope:** `src/bypass_mcu_pic10f320.c` only — both entries are
file-scoped, but each covers every finding with that ID attributed to this file.
...
analogous statement against the sibling PIC10F322 header from the same DFP pack
analyzes clean in `src/bypass_mcu_pic10f322.c`. The cause was bisected to this
file's heavy in-function `static_assert` use — stripping those lines makes both
diagnostics vanish with every other flag unchanged. The two "unused" macros are
consumed
...
**Scope control.** Both entries affect only the PIC10F320 source file, not other
analysis invocations. All other MISRA rule IDs remain visible; this was verified
by injecting a Rule 14.4 violation and confirming the lane fails. The Rule 2.5
entry necessarily also covers a future genuine Rule 2.5 finding in this file, so
changes to its macro set require an unsuppressed inventory review.

**Note for anyone tightening this.** The PIC10F322 run still suppresses
`misra-config` invocation-wide on its command line (see the PIC-specific notes
above). That suppression affects only the PIC10F322 invocation, not the AVR or
PIC10F320 lanes. Bringing it to file-scoped treatment is a worthwhile follow-up;
...
2. Run
   ...
   `--suppress=misra-config` when reviewing its complete inventory. An ordinary
   successful target discards suppressed findings and cannot serve as that review.
