...
- [x] One canonical expected-product set requires all three PIC10F320 images.
      ...
      *(`RELEASE_IMAGES` = 15 basenames, consumed by all three of the release
      ...
      `test_release_images.sh` 42 checks, including request-override independence;
      the global-omission negative test was
...
*(Status note, v0.9.8: the deferred naming work referenced throughout this
section is complete. All four axes -- release image basenames, output-stage
vocabulary, make-target prefixes and Makefile variable prefixes -- were paid off
on branch `hex-image-name-consistency`, and the `TODO.md` Tier 3 item that
tracked them was removed per that file's "completed work is deleted, not marked
DONE" convention. `CHANGELOG.md` and git history are the record.)*
...
| --- | --- | --- | --- |
| D1 *(superseded in v0.9.8: both lanes are now part-named, `pic10f322-*` / `pic10f320-*`, and a `pic-*` goal means both PIC parts)* | §5.1/§5.6 lane prefix | **Keep `pic-` = PIC10F322**; new lane is `pic320-` with `PIC320_*` variables. The 322 lane is not renamed. | No churn on known-good 322 targets and no interface break for existing `make print-PIC_*` consumers. Accepts the asymmetry §5.1 flags: `pic-` silently means "the other PIC". Because `PIC_*` stays 322, §5.6's rule reads "anything chip-specific and new gets `PIC320_`", and the shared-tool allowlist in §5.6 stays exactly as written. |
