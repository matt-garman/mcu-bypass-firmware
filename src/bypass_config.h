...
// no host/test code references TIMER0_OCR0A_1MS or the TIMSK/TIFR aliases.)
#if defined(__AVR__)


// AVR-XT (avrxmega3 / tinyAVR-0/1, e.g. ATtiny202) also defines __AVR__, but is
// register-incompatible with classic AVR: the 1ms tick is TCB0 (not Timer0/CTC)
// and there is no TIMSK0/TIFR0. Select it FIRST so it never falls into the
// classic arm's F_CPU #error or the TIMSK/TIFR aliases below.
#if defined(BYPASS_MCU_AVR_XT)

#  if !defined(F_CPU)
#    error "F_CPU must be defined via build flags, e.g. -DF_CPU=2000000UL"
#  endif
#  if (F_CPU != 2000000UL)
#    error "F_CPU must be 2000000 for the ATtiny202 (16MHz OSCCFG, MCLKCTRLB PDIV 8)"
#  endif

// TCB0 periodic-interrupt compare for a 1ms tick. The counter runs 0..CCMP
// inclusive => (CCMP + 1) CLK_PER cycles per interrupt, so CCMP = (F_CPU/1000)-1
